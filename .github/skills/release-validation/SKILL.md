...
4. Verify committed lockfile URLs use only `https://registry.npmjs.org/`; scan tracked files for proxy URLs, credentials, tokens, and local registry configuration.
5. Test the pre-commit normalizer against already-canonical, public noncanonical, Microsoft-proxy-style, scoped, staged-only, and mixed staged/unstaged lockfile cases.
